...
export class GrpcClient {
  ...
  // Função para enviar o trace_id nos metadados
  private sendTraceId(): string {
  ...
  public async getPrice(name: string): Promise<any> {
    const traceId = this.sendTraceId();
  
    const metadata = new grpc.Metadata();
    metadata.set('trace_id', traceId);
  
    // Retorna uma Promise que resolve com a resposta ou rejeita com o erro
